...
const handleSubmit = async(e)=>{
  ...
  return (
    <div>
      <section className="bg-white my-10  text-tex flex   flex-col items-center h-[120vh] justify-center   ">
        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0 h-full w-full ">
          ...
          <div className="w-full bg-white rounded-3xl   shadow dark:border md:mt-0 sm:max-w-md xl:p-0 ">
            ...
            <div className="w-full flex justify-center pr-5 max-md:pr-0 ">
              ...
              <img src={logo} className='w-[70px] max-md:w-[40px] pb-5  '  alt="" />
              </div>
              ...
              <form className=" pt-5 h-full " action="#" onSubmit={handleSubmit} method="post">
               {
                ...
                inputs.map((inp)=>{
                  return (
                    <>
                    <div>
                      <label htmlFor={inp.lable} className="block mb-2 max-md:mb-0.5 ml-1  text-sm font-bold  text-gray-500 max-md:text-[11px] ">{inp.lable}</label>
                      <input onChange={handleChange} type={inp.type} name={inp.name} id="email" value={inp.value} className="border mb-3  shadow-sm drop-shadow-sm max-md:placeholder:text-[8px] max-md:p-1.5   max-md:text-xs max-sm:text-[10px]  placeholder:text-xs  shadow-slate-100  text-gray-700 sm:text-sm rounded-xl  outline-none block w-full p-2.5 dark:placeholder-gray-300  dark:focus:ring-lgrn dark:focus:border-lgrn" placeholder={inp.palce}  />
                    </div>
                ...
                <button type="submit" className=" mt-4 bg-lgrn text-white rounded-full w-full h-9 mb-4  max-md:h-7 max-md:text-xs shadow-md drop-shadow-md  hover:bg-grn transition duration-300">Create New Acount</button>
